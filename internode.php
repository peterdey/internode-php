<?php
  ...
  // Number of recent days to graph data for. (0 = default = 1 year)
  define("GRAPH_DAYS", 0);
  ...
  class internode {
    ...
    function refresh_cache() {
      ...
      $data = $this->fetch_data($url . '/history' . $count);
    ...
    function fetch_data($url) {
      ...
      if ($info['http_code'] != 200) {
        if ($info['http_code'] == 401) {
          ...
          echo "<p>Please check that the username you have set does not end in <em>@internode.on.net</em> and that your password is correct.\n";
        } else {
          echo "<h3>Whoops!</h3>\n";
          echo "<p>There was a problem fetching your usage data. The Internode API server returned HTTP status code {$info['http_code']}. You can look up what that means at <a href=\"http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html#sec10.2\">http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html#sec10.2</a>\n";
        }
        echo "<p>The full error message as provided by Internode is:\n";
        echo $result;
        die();
      }
    ...
    function display_history() {
      ...
      for($i = 0; $i < count($this->history); $i++) {
        ...
        if (isset($i) && ($i > 0) && count($this->history) > 90) {
          for($j = ($i-44); $j <= ($i+44); $j++) {
            if( isset($this->history[$j]) && $this->history[$j] ) {
	      if (isset($this->history[$j]->usage)) {
      ...
      if(!$this->unlimited) {
        ...
        if($this->remaining > 0) {
          $string = sprintf("Daily Remaining: %9s   Total Remaining: %9s", format_size($this->remaining / $this->days_remaining), format_size($this->remaining) );
      ...
      $generator = sprintf("Fetched %s", date("d M Y H:i:s", filemtime(INTERNODE_CACHE)) );
      imagestring($im, 1, (IMAGE_BORDER_LEFT+IMAGE_WIDTH+(2*IMAGE_BORDER))/2 - imagefontwidth(1) * (strlen($generator)/2), IMAGE_HEIGHT+IMAGE_BORDER_BOTTOM+IMAGE_BORDER, $generator, $black);
